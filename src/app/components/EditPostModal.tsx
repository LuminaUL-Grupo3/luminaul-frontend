...
import type { PostType, PostUpdatePayload } from '../../types/post';
...
export function EditPostModal({ isOpen, onClose, post, onSave }: EditPostModalProps) {
  ...
  const onSubmit = async (data: FormData) => {
    const payload: PostUpdatePayload = {
      ...
      group_id: post.type === 'tutoring' ? null : data.studyGroup,
    };
    ...
    try {
      ...
      setSaveError(err instanceof Error ? err.message : 'No se pudo guardar la publicación.');
    } finally {
      setSaving(false);
    }
  };
