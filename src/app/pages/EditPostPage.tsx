...
import type { PostUpdatePayload } from '../../types/post';
...
export function EditPostPage() {
  ...
  const { post, status, error, save } = usePostEditor(id ?? null);
  ...
  // precargar el formulario cuando llega el post real (GET /api/posts/:id)
  useEffect(() => {
    if (post) {
      reset({
        postType: post.type === 'tutoring' ? 'asesoria' : 'grupo',
        course: post.course_id,
        studyGroup: post.group_id ?? '',
  ...
  const onSubmit = async (data: FormData) => {
    const payload: PostUpdatePayload = {
      ...
      type: data.postType === 'asesoria' ? 'tutoring' : 'study_group',
      description: data.description,
      group_id: data.postType === 'asesoria' ? null : data.studyGroup,
    };
    ...
    try {
      ...
      // el error ya queda reflejado en `status === "error"` / `error`
    }
  };
    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-8rem)]">
        <div className="bg-white rounded-xl border border-border p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-2xl mb-2">No se pudo cargar la publicación</h2>
          <p className="text-muted-foreground mb-6">
            {error ?? 'Es posible que ya no exista o haya sido eliminada.'}
          </p>
          <button
            onClick={() => navigate('/mis-publicaciones')}
            className="px-6 h-12 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors"
          >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            <button
              ...
              className="flex-1 h-12 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50"
            >
